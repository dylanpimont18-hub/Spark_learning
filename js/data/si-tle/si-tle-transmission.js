    ...
    exercice: {
      ...
      generate() {
        ...
        if (scenario === 'vitesse') {
          ...
          return {
            statement: `Un réducteur ($|r| = 1/${factor}$, $\\eta = ${fr(eta, 2)}$) reçoit un couple moteur $C_e = ${Ce}$ N·m. Calcule le couple de sortie $C_s$ (en N·m).`,
            answer: Cs,
            tolerance: 0.5,
            unit: 'N·m',
            hint: `$C_s = \\dfrac{\\eta}{|r|} \\times C_e = \\dfrac{${fr(eta, 2)}}{1/${factor}} \\times ${Ce}$.`,
            solution: [
              `$C_s = \\dfrac{\\eta}{|r|} \\times C_e = \\dfrac{${fr(eta, 2)}}{1/${factor}} \\times ${Ce}$`,
              `$C_s = ${fr(eta, 2)} \\times ${factor} \\times ${Ce}$`,
              `$C_s = ${fr(Cs)}$ N·m`
            ]
          };
        } else {
