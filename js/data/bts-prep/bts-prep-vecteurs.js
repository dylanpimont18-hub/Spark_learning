    ...
    definitions: [
      {
        ...
        def: `Un vecteur $\\vec{u}$ dans le plan est défini par ses composantes dans un repère orthonormé $(O, \\vec{x}, \\vec{y})$ :<br/>
$$\\vec{u} = \\begin{pmatrix} u_x \\\\ u_y \\end{pmatrix} = u_x\\,\\vec{x} + u_y\\,\\vec{y}$$<br/><br/>
La <strong>norme</strong> (longueur) : $\\|\\vec{u}\\| = \\sqrt{u_x^2 + u_y^2}$<br/>
L'<strong>angle</strong> par rapport à l'axe horizontal : $\\theta = \\arctan\\left(\\frac{u_y}{u_x}\\right)$`,
      },
      {
        term: 'Opérations sur les vecteurs',
        def: `<em>Addition (résultante) :</em> $\\vec{u} + \\vec{v} = \\begin{pmatrix} u_x + v_x \\\\ u_y + v_y \\end{pmatrix}$<br/><br/>
<em>Multiplication par un scalaire :</em> $k\\,\\vec{u} = \\begin{pmatrix} k\\,u_x \\\\ k\\,u_y \\end{pmatrix}$<br/><br/>
<em>Vecteur opposé :</em> $-\\vec{u} = \\begin{pmatrix} -u_x \\\\ -u_y \\end{pmatrix}$`,
      },
      {
        term: 'Produit scalaire',
        def: `Le produit scalaire est un <em>nombre</em> (pas un vecteur) :<br/>
...
• Si $\\vec{u} \\cdot \\vec{v} = \\|\\vec{u}\\| \\times \\|\\vec{v}\\|$ → vecteurs <strong>colinéaires de même sens</strong>`,
    ...
    method: {
      ...
      steps: [
        'Décomposer chaque force : $F_x = F\\cos\\theta$ (horizontale) et $F_y = F\\sin\\theta$ (verticale)',
        ...
        'Travail d\'une force (produit scalaire) : $W = \\vec{F} \\cdot \\vec{d} = F \\times d \\times \\cos\\theta$ (J)',
      ],
    },

    example: {
      statement: 'Vecteurs — 3 exemples techniques résolus',
      steps: [
        '<strong>Exemple 1 — Résultante de deux câbles</strong> : $F_1 = 800\\;\\text{N}$ à 60°, $F_2 = 600\\;\\text{N}$ à 120° → $R_x = 400 - 300 = 100\\;\\text{N}$, $R_y = 693 + 520 = 1213\\;\\text{N}$ → $R = \\sqrt{100^2 + 1213^2} \\approx 1217\\;\\text{N}$ à $\\alpha \\approx 85{,}3°$.',
        '<strong>Exemple 2 — Champ électrique résultant</strong> : $\\vec{E_1} = (300, 0)$ V/m et $\\vec{E_2} = (0, 400)$ V/m → $\\vec{E} = (300, 400)$ → $E = \\sqrt{300^2 + 400^2} = 500\\;\\text{V/m}$.',
        '<strong>Exemple 3 — Poussée d\'Archimède vs poids</strong> : vecteurs colinéaires et opposés → $\\vec{\\Pi} \\cdot \\vec{P} = \\Pi P \\cos(180°) = -\\Pi P$. Si $\\Pi > P$ : flotteur monte ; si $\\Pi < P$ : il coule.',
      ],
      answer: 'Règle : on additionne les composantes (jamais les normes). Produit scalaire nul ↔ perpendiculaire. Produit scalaire = ‖u‖‖v‖ ↔ colinéaire même sens.',
    },
...
<pre style="margin:0;color:var(--text)">
  ...
  F   = √(F_x² + F_y²)  (retour à la norme)
    ...
    recap: [
      ...
      'Addition composante par composante — ne jamais additionner les normes directement',
      'Produit scalaire : $\\vec{u}\\cdot\\vec{v} = u_x v_x + u_y v_y = \\|u\\|\\|v\\|\\cos\\theta$',
      '$\\vec{u}\\perp\\vec{v}$ ↔ $\\vec{u}\\cdot\\vec{v} = 0$',
      'Décomposition d\'une force en $F\\cos\\theta$ (horizontale) et $F\\sin\\theta$ (verticale)',
    ],
