...
Exemples concrets : la force exercée par un câble de levage, la vitesse d'un fluide dans une canalisation, le champ électrique créé par un conducteur, la poussée d'Archimède sur un sous-marin.`,
    ...
    definitions: [
      { term: 'Représentation d\'un vecteur', def: '$\\vec{u} = \\begin{pmatrix} u_x \\\\ u_y \\end{pmatrix}$ — Norme : $\\|\\vec{u}\\| = \\sqrt{u_x^2 + u_y^2}$<br/>Angle par rapport à l\'horizontal : $\\theta = \\arctan(u_y/u_x)$' },
      { term: 'Opérations vectorielles', def: 'Addition : $\\vec{u} + \\vec{v} = (u_x+v_x,\\; u_y+v_y)$<br/>Multiplication scalaire : $k\\vec{u} = (ku_x,\\; ku_y)$<br/>Vecteur opposé : $-\\vec{u} = (-u_x,\\; -u_y)$' },
      { term: 'Produit scalaire', def: '$\\vec{u} \\cdot \\vec{v} = u_x v_x + u_y v_y = \\|\\vec{u}\\| \\times \\|\\vec{v}\\| \\times \\cos\\theta$ (scalaire, pas un vecteur !)<br/>$\\vec{u} \\cdot \\vec{v} = 0$ → perpendiculaires ; $= \\|u\\|\\|v\\|$ → colinéaires de même sens.' },
    ],

    method: {
      title: 'Décomposer et composer des vecteurs forces',
      steps: [
        'Force $F$ à l\'angle $\\theta$ : $F_x = F\\cos\\theta$, $F_y = F\\sin\\theta$.',
        'Pour la résultante de plusieurs forces : décomposer chaque force, additionner composante par composante ($R_x = \\sum F_{x,i}$, $R_y = \\sum F_{y,i}$).',
        'Calculer la norme : $R = \\sqrt{R_x^2 + R_y^2}$.',
        'Calculer l\'angle de la résultante : $\\alpha = \\arctan(R_y / R_x)$.',
        '<strong>Travail d\'une force</strong> : $W = \\vec{F} \\cdot \\vec{d} = F \\times d \\times \\cos\\theta$ (J), où $\\theta$ est l\'angle entre la force et le déplacement.',
      ],
    },

    example: {
      statement: 'Trois applications vectorielles : résultante de câbles, champ électrique, poussée d\'Archimède.',
      steps: [
        '<strong>Mécanique : résultante de deux câbles</strong><br/>$F_1 = 800\\;\\text{N}$ à 60°, $F_2 = 600\\;\\text{N}$ à 120°<br/>$R_x = 800\\cos 60° + 600\\cos 120° = 400 - 300 = 100\\;\\text{N}$<br/>$R_y = 800\\sin 60° + 600\\sin 120° = 693 + 520 = 1213\\;\\text{N}$<br/>$R = \\sqrt{100^2 + 1213^2} \\approx 1217\\;\\text{N}$, $\\alpha \\approx 85{,}3°$',
        '<strong>Électrotechnique : champ résultant</strong><br/>$\\vec{E_1} = (300, 0)$ V/m et $\\vec{E_2} = (0, 400)$ V/m<br/>$\\vec{E} = (300, 400)$ V/m — $E = \\sqrt{300^2 + 400^2} = 500\\;\\text{V/m}$',
        '<strong>Thermique : poussée d\'Archimède</strong><br/>$\\vec{\\Pi}$ (vertical ↑) et $\\vec{P}$ (vertical ↓) sont colinéaires et opposés.<br/>$\\vec{\\Pi} \\cdot \\vec{P} = \\Pi \\times P \\times \\cos 180° = -\\Pi P$<br/>Si $\\Pi > P$ → flotte ; si $\\Pi < P$ → coule.',
      ],
      answer: 'Additionner les composantes séparément, jamais les normes directement. Le produit scalaire donne le travail d\'une force ($\\text{N·m} = \\text{J}$).',
    },
    ...
    formulas: [
      ...
      '<strong>Décomposition d\'une force</strong> : $F_x = F\\cos\\theta$, $F_y = F\\sin\\theta$',
      '<strong>Travail d\'une force</strong> : $W = \\vec{F}\\cdot\\vec{d} = F\\,d\\cos\\theta$ (J)',
    ],
...
<pre style="margin:0;color:var(--text)">
  ...
  F   = √(F_x² + F_y²)  (retour à la norme)
    ...
    recap: [
      ...
      'Addition composante par composante : $(u_x + v_x,\\; u_y + v_y)$.',
      'Produit scalaire : $\\vec{u}\\cdot\\vec{v} = u_x v_x + u_y v_y = \\|\\vec{u}\\|\\,\\|\\vec{v}\\|\\cos\\theta$.',
      '$\\vec{u}\\perp\\vec{v}$ si et seulement si $\\vec{u}\\cdot\\vec{v} = 0$.',
      'Décomposition d\'une force : composante horizontale $F\\cos\\theta$, verticale $F\\sin\\theta$.',
    ],
