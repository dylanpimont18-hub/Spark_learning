    ...
    definitions: [
      {
        ...
        def: `$\\vec{u} = \\begin{pmatrix} u_x \\\\ u_y \\end{pmatrix} = u_x\\,\\vec{x} + u_y\\,\\vec{y}$ dans un repère orthonormé.<br/><br/>
...
<strong>Angle</strong> par rapport à l'horizontal : $\\theta = \\arctan\\left(\\dfrac{u_y}{u_x}\\right)$`,
      },
      {
        ...
        def: `<em>Addition</em> : $\\vec{u} + \\vec{v} = (u_x + v_x,\\; u_y + v_y)$<br/>
<em>Scalaire</em> : $k\\,\\vec{u} = (k u_x,\\; k u_y)$<br/>
<em>Opposé</em> : $-\\vec{u} = (-u_x,\\; -u_y)$`,
      },
      {
        term: 'Produit scalaire',
        def: `$\\vec{u} \\cdot \\vec{v} = u_x v_x + u_y v_y = \\|\\vec{u}\\| \\times \\|\\vec{v}\\| \\times \\cos\\theta$ — c'est un <em>nombre</em> (scalaire).<br/><br/>
• $\\vec{u} \\cdot \\vec{v} = 0$ → vecteurs <strong>perpendiculaires</strong><br/>
• $\\vec{u} \\cdot \\vec{v} = \\|\\vec{u}\\|\\|\\vec{v}\\|$ → vecteurs <strong>colinéaires, même sens</strong>`,
      },
    ],

    method: {
      title: 'Décomposition et résultante de forces',
      steps: [
        '<strong>Décomposer une force</strong> : $F_x = F\\cos\\theta$ (horizontale) · $F_y = F\\sin\\theta$ (verticale)',
        'Décomposer chaque force en composantes $(F_{x,i},\\; F_{y,i})$',
        'Additionner composante par composante : $R_x = \\sum F_{x,i}$ · $R_y = \\sum F_{y,i}$',
        'Calculer la norme : $R = \\sqrt{R_x^2 + R_y^2}$ et l\'angle : $\\alpha = \\arctan(R_y / R_x)$',
        '<strong>Travail d\'une force</strong> : $W = \\vec{F} \\cdot \\vec{d} = F \\times d \\times \\cos\\theta$ (J) — où $\\theta$ est l\'angle entre $\\vec{F}$ et $\\vec{d}$.',
      ],
    },

    example: {
      statement: 'Calcul vectoriel appliqué : résultante de câbles, champ électrique, poussée d\'Archimède.',
      steps: [
        '<strong>Exemple 1 — Résultante de deux câbles</strong> : $F_1 = 800\\;\\text{N}$ à 60°, $F_2 = 600\\;\\text{N}$ à 120° → $R_x = 400 - 300 = 100\\;\\text{N}$ | $R_y = 693 + 520 = 1213\\;\\text{N}$ | $R = \\sqrt{100^2 + 1213^2} \\approx 1217\\;\\text{N}$ à 85{,}3°',
        '<strong>Exemple 2 — Champ électrique résultant</strong> : $\\vec{E_1} = (300, 0)$ V/m, $\\vec{E_2} = (0, 400)$ V/m → $\\vec{E} = (300, 400)$ V/m | $E = \\sqrt{300^2+400^2} = 500\\;\\text{V/m}$',
        '<strong>Exemple 3 — Poussée d\'Archimède</strong> : $\\vec{\\Pi}$ (haut) et $\\vec{P}$ (bas) colinéaires opposés → $\\vec{\\Pi} \\cdot \\vec{P} = \\Pi P \\cos(180°) = -\\Pi P$. Si $\\Pi > P$ : flotte ; si $\\Pi < P$ : coule.',
      ],
      answer: 'On ne peut pas additionner des normes directement : $\\|\\vec{F_1}+\\vec{F_2}\\| \\neq \\|\\vec{F_1}\\|+\\|\\vec{F_2}\\|$. Il faut toujours passer par les composantes.',
    },
