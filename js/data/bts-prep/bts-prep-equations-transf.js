...
Si $A \\cdot \\ln(Bx) = C$ : $\\ln(Bx) = C/A$ → $Bx = e^{C/A}$ → $x = e^{C/A} / B$.`,
    ...
    method: {
      ...
      steps: [
        `<strong>Règle d'or :</strong> appliquer l'opération inverse de chaque côté de l'équation.<br/><br/>
$\\sqrt{x}$ → mettre au carré &nbsp;·&nbsp; $x^2$ → racine carrée &nbsp;·&nbsp; $x^n$ → puissance $1/n$<br/>
$e^x$ → logarithme naturel $\\ln()$ &nbsp;·&nbsp; $\\ln(x)$ → exponentielle $e^{()}$ &nbsp;·&nbsp; $\\log_{10}(x)$ → $10^{()}$`,
        `<strong>Domaines à respecter :</strong><br/>
• $\\sqrt{x}$ n'existe que si $x \\geq 0$<br/>
• $\\ln(x)$ n'existe que si $x > 0$<br/>
• $x^2 = a$ a deux solutions ($\\pm\\sqrt{a}$) : choisir la solution physiquement cohérente (vitesse, résistance, longueur sont positives)`,
      ],
    },

    example: {
      statement: `Retrouver $R$ dans $Z = \\sqrt{R^2 + X_L^2}$ avec $Z = 100\;\\Omega$, $X_L = 60\;\\Omega$`,
      steps: [
        '$100 = \\sqrt{R^2 + 60^2}$',
        '$100^2 = R^2 + 3600$ (mise au carré des deux membres)',
        '$R^2 = 10000 - 3600 = 6400$',
        '$R = \\sqrt{6400} = 80\;\\Omega$',
        `<strong>Exemple 2 — Vitesse depuis $E_c = \\frac{1}{2}mv^2$</strong> ($E_c = 3600\;\\text{J}$, $m = 80\;\\text{kg}$) :<br/>$v^2 = 3600 / 40 = 90$ → $v = \\sqrt{90} \\approx 9{,}49\;\\text{m/s}$`,
        `<strong>Exemple 3 — Loi RC inverse :</strong> $u_C(t) = 12(1 - e^{-t/0{,}5})$, trouver $t$ pour $u_C = 9\;\\text{V}$<br/>$e^{-t/0{,}5} = 0{,}25$ → $-t/0{,}5 = \\ln(0{,}25)$ → $t = 0{,}693\;\\text{s}$`,
        `<strong>Exemple 4 — dB :</strong> filtre à $-30\;\\text{dB}$ en tension → $V_s/V_e = 10^{-1{,}5} \\approx 0{,}0316$`,
      ],
      answer: '$R = 80\;\\Omega$ — méthode : mise au carré des deux membres, puis racine carrée finale.',
    },
