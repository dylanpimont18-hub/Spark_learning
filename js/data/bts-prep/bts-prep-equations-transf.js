...
Si $\\log_{10}(x) = a$, alors $x = 10^a$.<br/>
Si $A \\cdot \\ln(Bx) = C$ : $\\ln(Bx) = C/A$ → $Bx = e^{C/A}$ → $x = e^{C/A} / B$.`,
    ...
    method: {
      ...
      steps: [
        'Identifier l\'opération qui bloque l\'inconnue ($\\sqrt{}$, puissance, $e^x$, $\\ln$, $\\log_{10}$…)',
        'Appliquer l\'opération inverse des deux côtés de l\'équation (tableau : $\\sqrt{x}$ → carré, $e^x$ → $\\ln$, $\\ln x$ → $e^{()}$, $\\log_{10} x$ → $10^{()}$)',
        'Vérifier le domaine de validité : $\\sqrt{x}$ nécessite $x \\geq 0$ ; $\\ln(x)$ nécessite $x > 0$',
        'Pour $x^2 = a$ : deux solutions $\\pm\\sqrt{a}$ → garder la solution physiquement cohérente',
        'Vérifier la solution dans l\'équation originale (surtout après mise au carré)',
      ],
    },

    example: {
      statement: 'Transformations inverses — 5 exemples techniques résolus',
      steps: [
        '<strong>Exemple 1 — Retrouver R ($Z = \\sqrt{R^2 + X_L^2}$)</strong> : $Z = 100\\;\\Omega$, $X_L = 60\\;\\Omega$ → mettre au carré : $R^2 = 100^2 - 60^2 = 6400$ → $R = \\sqrt{6400} = 80\\;\\Omega$',
        '<strong>Exemple 2 — Vitesse depuis $E_c = \\frac{1}{2}mv^2$</strong> : $E_c = 3600\\;\\text{J}$, $m = 80\\;\\text{kg}$ → $v^2 = 3600 / 40 = 90$ → $v = \\sqrt{90} \\approx 9{,}49\\;\\text{m/s}$',
        '<strong>Exemple 3 — Inverser une loi RC</strong> : $u_C(t) = 12(1 - e^{-t/0{,}5}) = 9\\;\\text{V}$ → $e^{-t/0{,}5} = 0{,}25$ → $t = 0{,}5 \\times \\ln(4) = 0{,}693\\;\\text{s}$',
        '<strong>Exemple 4 — Rapport de tension depuis dB</strong> : $-30 = 20\\log_{10}(V_s/V_e)$ → $\\log_{10}(V_s/V_e) = -1{,}5$ → $V_s/V_e = 10^{-1{,}5} \\approx 0{,}0316$',
        '<strong>Exemple 5 — Pression dynamique (CVC)</strong> : $p_d = 20\\;\\text{Pa}$, $\\rho = 1{,}2\\;\\text{kg/m}^3$ → $v = \\sqrt{2 \\times 20 / 1{,}2} = \\sqrt{33{,}33} \\approx 5{,}77\\;\\text{m/s}$',
      ],
      answer: 'Principe : appliquer l\'opération inverse des deux côtés, vérifier le domaine, retenir la solution physiquement cohérente.',
    },
