    ...
    definitions: [
      {
        ...
        def: `Si $\\sqrt{x} = a$ (avec $a > 0$), alors $x = a^2$.<br/><br/>
Si $A = \\sqrt{B^2 + C^2}$, alors $B = \\sqrt{A^2 - C^2}$ (à condition que $A > C$).`,
      },
      {
        term: 'Inverser une puissance',
        def: `Si $x^n = a$, alors $x = a^{1/n} = \\sqrt[n]{a}$.<br/><br/>
Cas courants :<br/>
$x^2 = a \\Rightarrow x = \\pm\\sqrt{a}$ (ne garder que la solution physique)<br/>
$x^3 = a \\Rightarrow x = \\sqrt[3]{a} = a^{1/3}$`,
      },
      {
        term: 'Inverser une exponentielle',
        def: `Si $e^x = a$, alors $x = \\ln(a)$.<br/><br/>
Si $A \\cdot e^{Bx} = C$ : isoler $e^{Bx} = C/A$, puis $Bx = \\ln(C/A)$, puis $x = \\dfrac{\\ln(C/A)}{B}$.`,
      },
      {
        term: 'Inverser un logarithme',
        def: `Si $\\ln(x) = a$, alors $x = e^a$.<br/>
Si $\\log_{10}(x) = a$, alors $x = 10^a$.<br/><br/>
Si $A \\cdot \\ln(Bx) = C$ : $\\ln(Bx) = C/A$ → $Bx = e^{C/A}$ → $x = e^{C/A} / B$.`,
      },
    ],

    method: {
      title: "Isoler l'inconnue par transformations successives",
      steps: [
        'Identifier la transformation qui affecte l\'inconnue ($\\sqrt{}$, $()^n$, $e^{()}$, $\\ln()$…)',
        'Isoler le terme contenant l\'inconnue d\'un côté de l\'équation',
        '<strong>Appliquer l\'opération inverse des deux côtés</strong> : $\\sqrt{x}$ → carré | $x^2$ → $\\sqrt{}$ | $e^x$ → $\\ln()$ | $\\ln(x)$ → $e^{()}$ | $\\log_{10}(x)$ → $10^{()}$',
        'Résoudre et vérifier la solution dans l\'équation d\'origine',
        '<strong>Domaines</strong> : $\\sqrt{x}$ exige $x \\geq 0$ | $\\ln(x)$ exige $x > 0$ | $x^2 = a$ donne $\\pm\\sqrt{a}$ (choisir la solution physique positive).',
      ],
    },

    example: {
      statement: 'Inverser des formules physiques : impédance, énergie cinétique, circuit RC, décibels, pression dynamique.',
      steps: [
        '<strong>Exemple 1 — $R$ dans $Z = \\sqrt{R^2 + X_L^2}$</strong> : $Z = 100\\;\\Omega$, $X_L = 60\\;\\Omega$ → $100^2 = R^2 + 3600$ → $R = \\sqrt{6400} = 80\\;\\Omega$',
        '<strong>Exemple 2 — $v$ depuis $E_c = \\frac{1}{2}mv^2$</strong> : $E_c = 3600\\;\\text{J}$, $m = 80\\;\\text{kg}$ → $v^2 = 3600/40 = 90$ → $v = \\sqrt{90} \\approx 9{,}49\\;\\text{m/s}$',
        '<strong>Exemple 3 — Inversion RC</strong> : $u_C = 12(1 - e^{-t/0{,}5}) = 9\\;\\text{V}$ → $e^{-t/0{,}5} = 0{,}25$ → $t = 0{,}5 \\times \\ln(4) \\approx 0{,}693\\;\\text{s}$',
        '<strong>Exemple 4 — Rapport de tension depuis dB</strong> : $-30\\;\\text{dB}$ → $\\log_{10}(V_s/V_e) = -1{,}5$ → $V_s/V_e = 10^{-1{,}5} \\approx 0{,}0316$',
        '<strong>Exemple 5 — Pression dynamique</strong> : $p_d = 20\\;\\text{Pa}$, $\\rho = 1{,}2\\;\\text{kg/m}^3$ → $v = \\sqrt{2 \\times 20 / 1{,}2} \\approx 5{,}77\\;\\text{m/s}$',
      ],
      answer: 'Dans chaque cas : identifier la transformation (racine, exponentielle, logarithme…), isoler le terme avec l\'inconnue, appliquer l\'opération inverse des deux côtés, vérifier la solution.',
    },
...
<div style="margin-top:10px;font-size:0.82rem;color:var(--text-muted)">
  Principe : appliquer l'inverse des deux côtés de l'égalité.
    ...
    recap: [
      'Pour inverser $\\sqrt{x}$ : <strong>mettre au carré</strong> les deux membres',
      'Pour inverser $e^x$ : prendre $\\ln$ des deux membres',
      'Pour inverser $\\ln(x)$ : prendre $e^{()}$ des deux membres',
      'Pour inverser $x^2$ : prendre $\\sqrt{}$ des deux membres (vérifier le signe physique)',
      'Dans les formules physiques : <strong>isoler</strong> le terme avec l\'inconnue, puis appliquer la transformation inverse',
    ],
