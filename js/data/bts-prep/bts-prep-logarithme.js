...
En BTS, vous les rencontrerez principalement sous deux formes : le <strong>logarithme décimal</strong> ($\\log_{10}$) pour les décibels et le pH, et le <strong>logarithme naturel</strong> ($\\ln$) pour les équations différentielles et les cinétiques.`,
    ...
    definitions: [
      {
        term: 'Exponentielle et logarithme naturel',
        def: `La fonction exponentielle $e^x$ est définie pour tout réel $x$. Sa base $e \\approx 2{,}718$ est la constante de Neper.<br/><br/>
Le logarithme naturel $\\ln(x)$ est la <strong>réciproque</strong> de $e^x$ :<br/>
$$\\ln(e^x) = x \\qquad e^{\\ln(x)} = x \\qquad (x > 0)$$`,
      },
      {
        term: 'Logarithme décimal',
        def: `Le logarithme décimal $\\log_{10}(x)$ est la réciproque de $10^x$ :<br/>
...
Relation : $\\ln(x) = \\log_{10}(x) \\times \\ln(10) \\approx 2{,}303 \\times \\log_{10}(x)$`,
      },
      {
        ...
        def: `$$\\ln(a \\times b) = \\ln a + \\ln b \\qquad \\ln\\left(\\frac{a}{b}\\right) = \\ln a - \\ln b \\qquad \\ln(a^n) = n \\times \\ln a$$<br/>
$$\\ln(1) = 0 \\qquad \\ln(e) = 1$$<br/><br/>
Exponentielle : $e^{a+b} = e^a \\times e^b$ &nbsp;·&nbsp; $(e^a)^n = e^{na}$ &nbsp;·&nbsp; $e^0 = 1$ &nbsp;·&nbsp; $e^{-x} = 1/e^x$`,
      },
      {
...
Si $a \\cdot e^{bx} = c$ → $e^{bx} = c/a$ → $bx = \\ln(c/a)$ → $x = \\ln(c/a)/b$`,
    ...
    method: {
      title: 'Les décibels (dB) et la constante de temps τ',
      steps: [
        `<strong>Niveau de puissance :</strong> $L_P = 10 \\times \\log_{10}\\left(\\dfrac{P}{P_0}\\right)\;\\text{dB}$<br/>
<strong>Niveau de pression/tension :</strong> $L_p = 20 \\times \\log_{10}\\left(\\dfrac{V}{V_0}\\right)\;\\text{dB}$<br/>
(Le facteur 20 vient de $P \\propto V^2$, donc $\\log(V^2/V_0^2) = 2\\log(V/V_0)$)`,
        '<strong>Valeurs repères dB :</strong> +3 dB ≈ doubler la puissance ; +6 dB ≈ doubler la pression ; +10 dB ≈ multiplier par 10 la puissance perçue.',
        `<strong>Constante de temps τ — systèmes RC et RL :</strong><br/>
Charge : $u_C(t) = E\\left(1 - e^{-t/\\tau}\\right)$ avec $\\tau = RC$<br/>
Décharge : $u_C(t) = U_0 \\cdot e^{-t/\\tau}$<br/>
À $t = \\tau$ : $u_C \\approx 0{,}632 \\times E$ (63,2%). À $t = 5\\tau$ : régime permanent (>99%).`,
      ],
    },

    example: {
      statement: `Électrotechnique : un filtre passe-bas atténue le signal d'un facteur 100 en tension. Calculer l'atténuation en dB.`,
      steps: [
        `$L = 20 \\times \\log_{10}(100) = 20 \\times 2 = 40\;\\text{dB}$`,
        'Le signal de sortie est à -40 dB par rapport à l\'entrée.',
        `<strong>Exemple 2 — Chimie (demi-vie) :</strong> réaction $[A](t) = [A]_0 \\cdot e^{-kt}$ avec $k = 0{,}05\;\\text{min}^{-1}$<br/>$t_{1/2} = \\dfrac{\\ln 2}{k} = \\dfrac{0{,}693}{0{,}05} = 13{,}9\;\\text{min}$`,
        `<strong>Exemple 3 — Thermique (NTU échangeur) :</strong> $\\varepsilon = 1 - e^{-\\text{NTU}} = 0{,}8$ → $e^{-\\text{NTU}} = 0{,}2$ → NTU $= -\\ln(0{,}2) = 1{,}61$`,
        `<strong>Exemple 4 — Acoustique :</strong> deux sources de 80 dB chacune → $L_{\\text{tot}} = 10\\log_{10}(10^8 + 10^8) = 10\\log_{10}(2 \\times 10^8) \\approx 83\;\\text{dB}$ (+3 dB)`,
      ],
      answer: '$L = -40\;\\text{dB}$ — atténuation d\'un facteur 100 en tension.',
    },
...
<div style="margin-top:10px;font-family:sans-serif;font-size:0.85rem;color:var(--text-muted)">
  τ = RC (en secondes si R en Ω et C en F)
    ...
    recap: [
      ...
      'Constante de temps : à $t = \\tau$, 63% de la valeur finale ; à $5\\tau$, régime permanent.',
