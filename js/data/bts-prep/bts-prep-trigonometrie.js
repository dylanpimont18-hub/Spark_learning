    ...
    definitions: [
      {
        ...
        def: `Par rapport à un angle $\\theta$ (différent de 90°) :<br/><br/>
$\\sin\\theta = \\dfrac{\\text{opposé}}{\\text{hypoténuse}}$ · $\\cos\\theta = \\dfrac{\\text{adjacent}}{\\text{hypoténuse}}$ · $\\tan\\theta = \\dfrac{\\text{opposé}}{\\text{adjacent}}$<br/><br/>
Mnémotechnique : <strong>SOH-CAH-TOA</strong><br/>
Identité fondamentale : $\\cos^2\\theta + \\sin^2\\theta = 1$`,
      },
      {
        term: "Unités d'angle : degrés et radians",
        def: `$\\theta_{\\text{rad}} = \\theta_{°} \\times \\dfrac{\\pi}{180}$ · $\\theta_{°} = \\theta_{\\text{rad}} \\times \\dfrac{180}{\\pi}$<br/><br/>
Tour complet = 360° = $2\\pi$ rad · Angle droit = 90° = $\\dfrac{\\pi}{2}$ rad`,
      },
      {
        term: 'Valeurs remarquables',
        def: `0° : sin = 0, cos = 1, tan = 0<br/>
30° : sin = $\\frac{1}{2}$, cos = $\\frac{\\sqrt{3}}{2}$, tan $\\approx 0{,}577$<br/>
45° : sin = cos = $\\frac{\\sqrt{2}}{2}$, tan = 1<br/>
60° : sin = $\\frac{\\sqrt{3}}{2}$, cos = $\\frac{1}{2}$, tan $\\approx 1{,}732$<br/>
90° : sin = 1, cos = 0, tan = indéfinie`,
      },
      {
        term: 'Le cercle trigonométrique',
        def: `Cercle de rayon 1 : un point sur le cercle a pour coordonnées $(\\cos\\theta, \\sin\\theta)$.<br/><br/>
Sin et cos sont <strong>périodiques</strong> de période $2\\pi$. Base des signaux AC : $u(t) = U_{\\max}\\sin(\\omega t + \\varphi)$.`,
      },
    ],
    ...
    method: {
      ...
      steps: [
        'Identifier l\'angle de référence $\\theta$ (différent de 90°)',
        'Nommer les côtés par rapport à $\\theta$ : opposé, adjacent, hypoténuse',
        'Choisir la fonction trigonométrique reliant les éléments connus à l\'inconnu (SOH-CAH-TOA)',
        'Isoler l\'inconnu et calculer',
        'Pour trouver un <strong>angle</strong> à partir de deux côtés : utiliser $\\arcsin$, $\\arccos$ ou $\\arctan$ (notées $\\sin^{-1}$, $\\cos^{-1}$, $\\tan^{-1}$ sur les calculatrices).',
      ],
    },

    example: {
      statement: 'Applications de la trigonométrie : angle de phase RL, pente de canalisation, efforts sur plan incliné.',
      steps: [
        '<strong>Exemple 1 — Circuit RL (électrotechnique)</strong> : $R = 60\\;\\Omega$, $X_L = 80\\;\\Omega$ → $Z = \\sqrt{3600+6400} = 100\\;\\Omega$ | $\\varphi = \\arctan(80/60) \\approx 53{,}1°$ | Vérification : $\\cos\\varphi = 60/100 = 0{,}6$ ✓',
        '<strong>Exemple 2 — Canalisation (BTP)</strong> : pente 2 % sur 25 m → $\\theta = \\arctan(0{,}02) \\approx 1{,}15°$ | dénivellation $h = 25 \\times 0{,}02 = 0{,}5\\;\\text{m}$',
        '<strong>Exemple 3 — Plan incliné (mécanique)</strong> : $m = 200\\;\\text{kg}$, $\\theta = 30°$, $P = 1962\\;\\text{N}$ → $P_{//} = P\\sin30° = 981\\;\\text{N}$ | $P_{\\perp} = P\\cos30° = 1699\\;\\text{N}$',
      ],
      answer: 'La trigonométrie permet de décomposer toute grandeur vectorielle en composantes : choisir SOH-CAH-TOA selon ce qu\'on connaît (côtés ou angle).',
    },
