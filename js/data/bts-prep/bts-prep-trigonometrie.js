...
Moyen mnémotechnique : <strong>SOH-CAH-TOA</strong>.<br/>
Identité fondamentale : $\\cos^2\\theta + \\sin^2\\theta = 1$`,
      },
      {
        term: 'Degrés et radians',
        def: `$$\\theta_{\\text{rad}} = \\theta_{°} \\times \\frac{\\pi}{180} \\qquad \\theta_{°} = \\theta_{\\text{rad}} \\times \\frac{180}{\\pi}$$<br/>
Un tour complet = 360° = $2\\pi$ rad. Un angle droit = 90° = $\\dfrac{\\pi}{2}$ rad.`,
      },
      {
        term: 'Valeurs remarquables',
        def: `| Angle | $\\sin$ | $\\cos$ | $\\tan$ |
|-------|--------|--------|--------|
| 0° | 0 | 1 | 0 |
...
| 90° | 1 | 0 | indéfinie |`,
      },
      {
        ...
        def: `Pour les angles supérieurs à 90° (signaux AC, rotations), cercle de rayon 1 centré à l'origine. Un point sur ce cercle a pour coordonnées $M = (\\cos\\theta,\\; \\sin\\theta)$.<br/><br/>
Les fonctions sin et cos sont périodiques de période $2\\pi$ (360°). Signaux sinusoïdaux : $u(t) = U_{\\max} \\sin(\\omega t + \\varphi)$.`,
      },
    ],
    ...
    method: {
      ...
      steps: [
        ...
        'Nommer les côtés : opposé (en face de θ), adjacent (touche θ sans être l\'hypoténuse), hypoténuse (en face du 90°)',
        'Choisir la fonction trigonométrique reliant les éléments connus et l\'inconnu (SOH-CAH-TOA)',
        'Isoler l\'inconnu et calculer',
        'Pour trouver un angle à partir de deux côtés : utiliser $\\arcsin$, $\\arccos$ ou $\\arctan$ (touches $\\sin^{-1}$, $\\cos^{-1}$, $\\tan^{-1}$ sur la calculatrice)',
      ],
    },

    example: {
      statement: 'Trigonométrie appliquée — 3 exemples techniques résolus',
      steps: [
        '<strong>Exemple 1 — Électrotechnique (angle de phase)</strong> : circuit RL avec $R = 60\\;\\Omega$, $X_L = 80\\;\\Omega$ → $Z = \\sqrt{60^2 + 80^2} = 100\\;\\Omega$ ; $\\varphi = \\arctan(80/60) \\approx 53{,}1°$. Vérification : $\\cos(53{,}1°) = 60/100 = 0{,}6$ ✓',
        '<strong>Exemple 2 — BTP (pente d\'une canalisation)</strong> : pente 2 % sur 25 m horizontaux → $\\theta = \\arctan(0{,}02) \\approx 1{,}15°$ ; dénivellation $h = 25 \\times 0{,}02 = 0{,}5\\;\\text{m}$.',
        '<strong>Exemple 3 — Mécanique (plan incliné à 30°)</strong> : masse $m = 200\\;\\text{kg}$, $P = 1962\\;\\text{N}$ → composante parallèle $P_{//} = 1962\\sin 30° = 981\\;\\text{N}$ ; composante perpendiculaire $P_\\perp = 1962\\cos 30° = 1699\\;\\text{N}$.',
      ],
      answer: 'SOH-CAH-TOA : identifier angle, opposé, adjacent, hypoténuse. Pour trouver un angle à partir de côtés : utiliser $\\arctan$, $\\arcsin$ ou $\\arccos$.',
    },
