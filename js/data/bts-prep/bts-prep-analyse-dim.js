...
• de comprendre les <strong>nombres adimensionnels</strong> utilisés en thermique et en mécanique des fluides`,
    ...
    definitions: [
      { term: 'Dimensions fondamentales', def: 'Symboles : M (Masse), L (Longueur), T (Temps), I (Courant), θ (Température), N (Quantité de matière).<br/>Notation : $[x]$ = dimension de la grandeur x.<br/>Exemples : $[v] = \\text{L·T}^{-1}$, $[F] = \\text{M·L·T}^{-2}$, $[W] = \\text{M·L}^2\\text{·T}^{-2}$, $[P_{\\text{élec}}] = \\text{M·L}^2\\text{·T}^{-3}$.' },
      { term: 'Principe d\'homogénéité', def: 'Une équation $A = B + C$ est homogène si et seulement si $[A] = [B] = [C]$.<br/><strong>On ne peut pas additionner des grandeurs de dimensions différentes.</strong> $5\\;\\text{m} + 3\\;\\text{kg}$ est physiquement absurde.' },
      { term: 'Nombres adimensionnels', def: 'Certains groupes de grandeurs forment des nombres sans dimension : leur valeur est indépendante du système d\'unités.<br/>Exemples : nombre de Reynolds $Re = \\rho v D / \\mu$, rendement $\\eta = P_{\\text{utile}} / P_{\\text{absorbée}}$, COP.' },
    ],
    ...
    method: {
      ...
      steps: [
        ...
        '<strong>Détermination d\'exposants</strong> : si $A = k \\cdot B^a \\cdot C^b$, écrire l\'équation dimensionnelle et identifier les puissances de chaque dimension pour trouver a et b.',
      ],
    },

    example: {
      statement: 'Quatre vérifications dimensionnelles appliquées à des formules de physique technique.',
      steps: [
        '<strong>Vérification : $E = \\frac{1}{2}mv^2$</strong><br/>$[m] = \\text{M}$, $[v] = \\text{L·T}^{-1}$<br/>$\\left[\\frac{1}{2}mv^2\\right] = \\text{M} \\times (\\text{L·T}^{-1})^2 = \\text{M·L}^2\\text{·T}^{-2} = [\\text{J}]$ ✓',
        '<strong>Détection d\'erreur : $P = F \\times v^2$ (faux !)</strong><br/>$[F \\times v^2] = \\text{M·L·T}^{-2} \\times \\text{L}^2\\text{·T}^{-2} = \\text{M·L}^3\\text{·T}^{-4} \\neq [\\text{W}]$ → formule <strong>fausse</strong>. La formule correcte est $P = F \\times v$.',
        '<strong>Électrotechnique : $U = \\sqrt{P \\times R}$</strong><br/>$[P \\times R] = \\text{M}^2\\text{·L}^4\\text{·T}^{-6}\\text{·I}^{-2}$<br/>$[\\sqrt{P \\times R}] = \\text{M·L}^2\\text{·T}^{-3}\\text{·I}^{-1} = [\\text{V}]$ ✓',
        '<strong>Hydraulique — Reynolds</strong><br/>$[Re] = \\dfrac{\\text{M·L}^{-3} \\times \\text{L·T}^{-1} \\times \\text{L}}{\\text{M·L}^{-1}\\text{·T}^{-1}} = 1$ → adimensionnel ✓',
      ],
      answer: 'Une formule homogène a les mêmes dimensions des deux membres. Une formule non homogène est <strong>forcément fausse</strong>.',
    },
