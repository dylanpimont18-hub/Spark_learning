...
• de comprendre les <strong>nombres adimensionnels</strong> utilisés en thermique et en mécanique des fluides`,
    ...
    definitions: [
      {
        term: 'Les dimensions fondamentales',
        def: `En analyse dimensionnelle, on utilise des symboles pour les dimensions (pas des unités) :<br/><br/>
<strong>L</strong> (longueur) · <strong>M</strong> (masse) · <strong>T</strong> (temps) · <strong>I</strong> (courant) · <strong>θ</strong> (température) · <strong>N</strong> (quantité de matière)<br/><br/>
Notation : $[x]$ désigne "la dimension de la grandeur x".<br/><br/>
Exemples : $[v] = \\text{L·T}^{-1}$ | $[F] = \\text{M·L·T}^{-2}$ | $[P_{\\text{press}}] = \\text{M·L}^{-1}\\text{·T}^{-2}$ | $[W] = \\text{M·L}^2\\text{·T}^{-2}$ | $[P_{\\text{élec}}] = \\text{M·L}^2\\text{·T}^{-3}$`,
      },
      {
...
$5\\;\\text{m} + 3\\;\\text{kg}$ est physiquement absurde.`,
      },
      {
        ...
        def: `Certains groupes de grandeurs forment des nombres sans dimension : ce sont des <em>nombres adimensionnels</em>. Ils ont la même valeur dans n'importe quel système d'unités.<br/><br/>
Exemples : nombre de Reynolds $Re = \\rho v D / \\mu$, rendement $\\eta = P_{\\text{utile}} / P_{\\text{absorbée}}$, COP.`,
      },
    ],

    method: {
      title: 'Vérification et détermination dimensionnelle',
      steps: [
        'Identifier toutes les grandeurs de la formule',
        'Écrire la dimension de chaque grandeur en termes de M, L, T, I, θ…',
        'Substituer dans la formule et simplifier',
        'Vérifier que les deux membres ont des dimensions identiques',
        '<strong>Détermination d\'une inconnue</strong> : si la formule est de la forme $A = k \\cdot B^a \\cdot C^b$, écrire l\'équation dimensionnelle et identifier les puissances de chaque dimension pour trouver les exposants.',
      ],
    },

    example: {
      statement: 'Vérifier par analyse dimensionnelle plusieurs formules de physique appliquée.',
      steps: [
        '<strong>Exemple 1 — $E_c = \\frac{1}{2}mv^2$</strong> : $[m] = \\text{M}$, $[v] = \\text{L·T}^{-1}$ → $\\left[\\frac{1}{2}mv^2\\right] = \\text{M·L}^2\\text{·T}^{-2} = [\\text{J}]$ ✓',
        '<strong>Exemple 2 — $P = F \\times v^2$ (faux !)</strong> : $[Fv^2] = \\text{M·L}^3\\text{·T}^{-4} \\neq \\text{M·L}^2\\text{·T}^{-3} = [\\text{W}]$ → formule <strong>fausse</strong>. La formule correcte est $P = F \\times v$.',
        '<strong>Exemple 3 — $U = \\sqrt{P \\times R}$ (électrotechnique)</strong> : $[\\sqrt{PR}] = \\sqrt{\\text{M}^2\\text{·L}^4\\text{·T}^{-6}\\text{·I}^{-2}} = \\text{M·L}^2\\text{·T}^{-3}\\text{·I}^{-1} = [\\text{V}]$ ✓',
        '<strong>Exemple 4 — Nombre de Reynolds</strong> : $[Re] = \\dfrac{[\\rho][v][D]}{[\\mu]} = \\dfrac{\\text{M·L}^{-3} \\times \\text{L·T}^{-1} \\times \\text{L}}{\\text{M·L}^{-1}\\text{·T}^{-1}} = 1$ → adimensionnel ✓',
      ],
      answer: 'L\'analyse dimensionnelle confirme $E_c = \\frac{1}{2}mv^2$ ✓ et $U = \\sqrt{PR}$ ✓, détecte l\'erreur $P = Fv^2$ (formule correcte : $P = Fv$), et prouve que $Re$ est adimensionnel ✓.',
    },
...
<div style="margin-top:12px;font-size:0.82rem;color:var(--text-muted)">
  Vérification : [U] = [R·I] = M·L²·T⁻³·I⁻²·I = M·L²·T⁻³·I⁻¹ ✓
    ...
    recap: [
      'Le <strong>principe d\'homogénéité</strong> : les deux membres d\'une équation doivent avoir les mêmes dimensions',
      'Notation : $[x]$ = dimension de x en termes de M, L, T, I, θ…',
      'Une formule <strong>homogène n\'est pas forcément juste</strong> (peut manquer un facteur numérique)',
      'Une formule <strong>non homogène est forcément fausse</strong>',
      'Un nombre <strong>adimensionnel</strong> a une valeur indépendante du système d\'unités',
    ],
