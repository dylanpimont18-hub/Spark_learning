    ...
    definitions: [
      {
        ...
        def: `• <strong>Axe des abscisses (x)</strong> : grandeur indépendante (cause). Ex : débit, température, fréquence.<br/>
...
• <strong>Titre et unités</strong> : indispensables pour donner un sens au graphique.`,
      },
      {
        term: 'Interpolation linéaire',
        def: `Si on connaît deux points $(x_1, y_1)$ et $(x_2, y_2)$, estimer $y$ pour $x$ intermédiaire :<br/><br/>
$$y = y_1 + \\frac{x - x_1}{x_2 - x_1} \\times (y_2 - y_1)$$`,
      },
      {
        term: "Pente d'une courbe",
        def: `$$a = \\frac{y_2 - y_1}{x_2 - x_1} \\qquad \\text{(unité = } [y]/[x]\\text{)}$$`,
      },
      {
        term: 'Extrapolation',
        def: `Estimer $y$ pour un $x$ en <strong>dehors</strong> de la plage connue. <strong>Risqué</strong> : la tendance peut changer hors plage. Toujours signaler qu'on extrapole et indiquer les limites de validité.`,
      },
      {
        term: 'Échelle logarithmique',
        def: `Les graduations sont espacées en proportion de $\\log(x)$.<br/>
• Droite sur graphe <strong>log-log</strong> → loi puissance : $y = a \\cdot x^n$<br/>
• Droite sur graphe <strong>semi-log</strong> (x lin, y log) → loi exponentielle`,
      },
    ],
    ...
    method: {
      ...
      steps: [
        ...
        `<strong>Pour calculer la pente d'une droite :</strong> choisir deux points éloignés sur la droite, lire $(x_1, y_1)$ et $(x_2, y_2)$, calculer $a = (y_2 - y_1)/(x_2 - x_1)$. Vérifier l'unité : $[a] = [y]/[x]$.`,
      ],
    },

    example: {
      statement: `Courbe de pompe Grundfos — interpolation pour $Q = 12\;\\text{m}^3/\\text{h}$ (entre les points à 10 et 15 m³/h)`,
      steps: [
        'Points connus : $(Q_1 = 10, H_1 = 38)$ et $(Q_2 = 15, H_2 = 27)$',
        `$\\text{HMT} = 38 + \\dfrac{12 - 10}{15 - 10} \\times (27 - 38) = 38 + \\dfrac{2}{5} \\times (-11) = 38 - 4{,}4 = 33{,}6\;\\text{m}$`,
        `<strong>Exemple 2 — Diagramme de Moody :</strong> abaque double échelle log. Pour $Re = 10^5$ et $\\varepsilon/D = 10^{-3}$, lecture graphique → $\\lambda \\approx 0{,}021$.`,
        `<strong>Exemple 3 — Caractéristique diode :</strong> points $(0{,}60\;\\text{V}, 10\;\\text{mA})$ et $(0{,}70\;\\text{V}, 80\;\\text{mA})$, pour $V = 0{,}65\;\\text{V}$ :<br/>$I = 10 + 0{,}5 \\times 70 = 45\;\\text{mA}$ (interpolation)`,
      ],
      answer: `$\\text{HMT} = 33{,}6\;\\text{m}$ pour $Q = 12\;\\text{m}^3/\\text{h}$ par interpolation linéaire.`,
    },
