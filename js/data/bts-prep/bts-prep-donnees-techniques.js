...
• <strong>Performances nominales</strong> : puissance, débit, pression, rendement, vitesse de rotation<br/>
...
• <strong>Courbes de performance</strong> : HMT/Q, rendement/Q, puissance/Q<br/>
• <strong>Conditions de référence</strong> : température ambiante, altitude, fluide<br/>
• <strong>Certifications</strong> : CE, NF, IP, classe d'isolation`,
      },
      {
        term: 'Pompes (ex : Grundfos)',
        def: `$Q_{\\text{nominal}}$ (m³/h) · $H_{\\text{nominal}}$ (m) · $P_{\\text{absorbée}}$ (kW) · $\\eta_{\\text{pompe}}$ (%) · $\\text{NPSH}_r$ (m) · DN raccordement (mm)`,
      },
      {
        term: 'Climatiseurs / Groupes froids (ex : Daikin)',
        def: `$P_f$ (kW) · $P_{\\text{chaud}}$ (kW) · COP · EER · $P_{\\text{élec}}$ (kW) · Classe énergétique (A+++ à G)`,
      },
      {
        term: 'Disjoncteurs / Contacteurs (ex : Schneider)',
        def: `$I_n$ (A) courant nominal · $I_{cu}$ (kA) pouvoir de coupure · catégorie d'emploi (AC-3, AC-1) · tension nominale (V)`,
      },
      {
        term: 'Chaudières industrielles',
        def: `$P_{\\text{utile}}$ (kW) · rendement PCI (%) · pression de travail (bar) · débit fumées (m³/h)`,
      },
    ],

    method: {
      title: 'Sélectionner un équipement dans un catalogue fabricant',
      steps: [
        ...
        '<strong>Vérifier les critères secondaires</strong> : rendement, puissance absorbée, NPSH, niveau sonore',
        ...
        '<strong>Indices IP XY</strong> : X = solides (0→6), Y = liquides (0→8). IP 54 = poussières + projections d\'eau. IP 65 = étanche poussières + jets d\'eau.',
      ],
    },

    example: {
      statement: 'Sélection d\'équipements à partir de catalogues fabricants : pompe de chauffage, climatiseur résidentiel, disjoncteur moteur.',
      steps: [
        '<strong>Exemple 1 — Pompe Grundfos CM5-8</strong> : besoin $Q = 8\\;\\text{m}^3/\\text{h}$, $H = 15\\;\\text{m}$ → point dans la plage de la CM5-8 ($Q_n = 9\\;\\text{m}^3/\\text{h}$, $H_n = 17\\;\\text{m}$, $P_{\\text{abs}} = 0{,}85\\;\\text{kW}$, $\\eta = 52\\%$, IP 44, 230 V mono).',
        '<strong>Exemple 2 — Climatiseur Daikin FTXM35M</strong> : $P_f = 3{,}5\\;\\text{kW}$, $P_{\\text{élec}} = 0{,}88\\;\\text{kW}$ → COP réel = $3{,}5/0{,}88 \\approx 4{,}0$ (catalogue annonce 5,0 en conditions STC), IP 24.',
        '<strong>Exemple 3 — Disjoncteur Schneider GV2M</strong> : moteur $P = 2{,}2\\;\\text{kW}$, $U = 400\\;\\text{V}$, $\\cos\\varphi = 0{,}85$ → $I_n = 2200/(1{,}732 \\times 400 \\times 0{,}85) \\approx 3{,}74\\;\\text{A}$ → GV2M08 réglé à 4 A.',
      ],
      answer: 'Méthode constante : identifier le point de fonctionnement, le placer sur l\'abaque, sélectionner le modèle avec marge de sécurité, vérifier NPSH / IP / compatibilité électrique.',
    },
...
<div style="font-family:monospace;color:var(--text)">
  ...
  Q_max = débit maxi (HMT = 0)
    ...
    recap: [
      ...
      '<strong>Point de fonctionnement</strong> pompe = intersection courbe pompe + courbe réseau',
      'IP XY : X = solides (0→6), Y = liquides (0→8). IP 65 = étanche poussières + jets d\'eau',
      '<strong>COP</strong> = puissance utile / puissance électrique absorbée',
