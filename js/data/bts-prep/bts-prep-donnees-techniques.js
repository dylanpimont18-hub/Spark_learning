...
• <strong>Certifications et normes</strong> : CE, NF, IP, classe d'isolation`,
      ...
      {
        term: 'Paramètres clés — Pompes (Grundfos)',
        def: `$Q_{\\text{nominal}}$ (m³/h), $H_{\\text{nominal}}$ (m), $P_{\\text{absorbée}}$ (kW), $\\eta_{\\text{pompe}}$ (%), $\\text{NPSH}_r$ (m), DN raccordement (mm)`,
      },
      {
        term: 'Paramètres clés — Climatiseurs / Groupes froids (Daikin)',
        def: `$P_f$ (kW), $P_{\\text{chaud}}$ (kW), COP, EER, $P_{\\text{élec}}$ (kW), Classe énergétique (A++ à G)`,
      },
      {
        term: 'Paramètres clés — Disjoncteurs / Contacteurs (Schneider)',
        def: `$I_n$ (A) courant nominal, $I_{cu}$ (kA) pouvoir de coupure, catégorie (AC-3, AC-1), tension nominale (V)`,
      },
      {
        term: 'Indices de protection IP',
...
IP 65 = étanche aux poussières (6) et aux jets d'eau basse pression (5).`,
    ...
    example: {
      statement: 'Lecture de fiches techniques — 3 exemples fabricants résolus',
      steps: [
        '<strong>Exemple 1 — Pompe Grundfos CM5-8</strong> : Besoin $(8\\;\\text{m}^3/\\text{h}, 15\\;\\text{m})$ → pompe CM5-8 couvre le point : $Q_n = 9\\;\\text{m}^3/\\text{h}$, $H_n = 17\\;\\text{m}$, $P_{\\text{abs}} = 0{,}85\\;\\text{kW}$, $\\eta = 52\\%$, IP 44, 230 V. Réserve de 1 m³/h sur le débit.',
        '<strong>Exemple 2 — Climatiseur Daikin FTXM35M</strong> : $P_f = 3{,}5\\;\\text{kW}$, $P_{\\text{élec}} = 0{,}88\\;\\text{kW}$ → COP réel = $3{,}5/0{,}88 \\approx 4{,}0$. La valeur catalogue COP = 5,0 est en conditions standard STC (35°C extérieur, 27°C intérieur).',
        '<strong>Exemple 3 — Disjoncteur Schneider GV2M</strong> : Moteur $P = 2{,}2\\;\\text{kW}$, $U = 400\\;\\text{V}$, $\\cos\\varphi = 0{,}85$ → $I_n = 2200/(1{,}732 \\times 400 \\times 0{,}85) \\approx 3{,}74\\;\\text{A}$ → sélectionner GV2M08 (réglable 4–6 A), réglé à 4 A.',
      ],
      answer: 'Vérifier toujours : performances nominales, plage de fonctionnement, conditions de référence et compatibilité électrique avant toute sélection.',
    },
