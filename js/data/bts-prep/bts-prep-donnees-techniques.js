    ...
    definitions: [
      {
        ...
        def: `• <strong>Référence et désignation</strong> : code produit, famille, variante<br/>
...
• <strong>Certifications et normes</strong> : CE, NF, IP, classe d'isolation`,
      ...
      {
        term: 'Paramètres pompes (ex : Grundfos)',
        def: `$Q_{\\text{nominal}}$ (m³/h), $H_{\\text{nominal}}$ (m), $P_{\\text{absorbée}}$ (kW), $\\eta_{\\text{pompe}}$ (%), $\\text{NPSH}_r$ (m), DN raccordement (mm)`,
      },
      {
        term: 'Paramètres groupes froids (ex : Daikin)',
        def: `$P_f$ (kW), $P_{\\text{chaud}}$ (kW), COP (chauffage), EER (refroidissement), $P_{\\text{élec}}$ (kW), Classe énergétique (A+++ à G)`,
      },
      {
        term: 'Paramètres protection IP',
        def: `IP XY : X = protection contre les solides (0 à 6), Y = protection contre les liquides (0 à 8).<br/>
<strong>IP 54</strong> = protégé contre les poussières (5) et les projections d'eau (4).<br/>
<strong>IP 65</strong> = étanche aux poussières (6) et aux jets d'eau basse pression (5).`,
      },
    ],
    ...
    method: {
      ...
      steps: [
        ...
        `<strong>Lire une courbe de pompe :</strong> la HMT décroît de gauche (Q=0, HMT max) à droite (Q max, HMT=0). Le point de fonctionnement est l'intersection entre la courbe pompe et la courbe de réseau.`,
      ],
    },

    example: {
      statement: `Sélection d'une pompe Grundfos CM pour alimenter un circuit de chauffage : $Q = 8\;\\text{m}^3/\\text{h}$ à $H = 15\;\\text{m}$`,
      steps: [
        `Sur l'abaque Grundfos, repérer le point $(8\;\\text{m}^3/\\text{h}, 15\;\\text{m})$.`,
        `La pompe CM5-8 couvre ce point : $Q_n = 9\;\\text{m}^3/\\text{h}$, $H_n = 17\;\\text{m}$, $P_{\\text{abs}} = 0{,}85\;\\text{kW}$, $\\eta_n = 52\\%$, IP 44, 230 V monophasé.`,
        `<strong>Exemple 2 — Climatiseur Daikin FTXM35M :</strong> $P_f = 3{,}5\;\\text{kW}$, $P_{\\text{élec}} = 0{,}88\;\\text{kW}$ → COP = $3{,}5/0{,}88 \\approx 4{,}0$ (valeur catalogue : 5,0 en conditions STC).`,
        `<strong>Exemple 3 — Disjoncteur Schneider GV2M :</strong> moteur 2,2 kW, 400 V, $\\cos\\varphi = 0{,}85$ → $I_n = 2200/(1{,}732 \\times 400 \\times 0{,}85) \\approx 3{,}74\;\\text{A}$ → sélectionner GV2M08 (4-6 A, réglé à 4 A).`,
      ],
      answer: `Pompe sélectionnée : CM5-8. Point de fonctionnement (8 m³/h, 15 m) dans la plage utile, avec 1 m³/h de réserve sur le débit.`,
    },
...
<div style="font-family:monospace;color:var(--text)">
  ...
  Q_max = débit maxi (HMT = 0)
    ...
    recap: [
      ...
      'IP XY : X = solides (0→6), Y = liquides (0→8). IP 65 = étanche poussières + jet d\'eau.',
      'COP = puissance utile / puissance électrique absorbée.',
      'NPSH : condition anti-cavitation — toujours vérifier : NPSH dispo ≥ NPSH requis + 0,5 m.',
    ],
